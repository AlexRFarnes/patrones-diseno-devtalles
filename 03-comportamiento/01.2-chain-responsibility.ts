...
class Supervisor extends BaseApprover {
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log(
        `Supervisor aprueba la solicitud de compra de %c$${amount}`,
        COLORS.green,
      );
      return;
    }
...
class Manager extends BaseApprover {
  override approveRequest(amount: number): void {
    if (amount <= 5000) {
      console.log(
        `Manager aprueba la solicitud de compra de %c$${amount}`,
        COLORS.yellow,
      );
      return;
    }
...
class Director extends BaseApprover {
  override approveRequest(amount: number): void {
    console.log(
      `Director aprueba la solicitud de compra de %c$${amount}`,
      COLORS.purple,
    );
  }
}
